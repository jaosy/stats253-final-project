...
covid <- covid %>%
  mutate(continent = recode(continent, "North America" = "North_America", "South America" = "South_America"))
```
...
- In addition to our existing variables, we also added the following to our model to predict the continent:
    ...
    - `median_age`
    - `gdp_per_capita`
- We also renamed the continent values to remove the space in "South America" and "North America" which makes it easier to code (e.g setting reference level)

```{r}
# creation of cv folds
set.seed(123)
covid_cv10 <- vfold_cv(covid, v = 4)
```
...
covid <- covid %>%
  mutate(continent = relevel(factor(continent), ref = 'North_America'))
...
# Recipe
logistic_rec <- recipe(continent ~ ., data = covid) %>%
