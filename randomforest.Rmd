...
data_wf_mtry23 <- workflow() %>%
  add_model(rf_spec %>% set_args(mtry = 23)) %>%
  add_recipe(rf_rec)

data_wf_mtry45 <- workflow() %>%
  add_model(rf_spec %>% set_args(mtry = 45)) %>%
  add_recipe(rf_rec)
```
...
set.seed(123) 
data_fit_mtry23 <- fit(data_wf_mtry23, data = covid)

set.seed(123)
data_fit_mtry45 <- fit(data_wf_mtry45, data = covid)
```
```{r}
...
data_rf_OOB_output <- bind_rows(
    ...
    rf_OOB_output(data_fit_mtry23,'mtry23', covid %>% pull(continent)),
    rf_OOB_output(data_fit_mtry45,'mtry45', covid %>% pull(continent))
)
